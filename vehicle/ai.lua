...
local OvertakeManager = {
  overtaking = false,  -- Whether the AI is currently overtaking
  overtakeSide = 0,    -- -1 for left, 1 for right
  overtakeStartTime = 0,  -- Time when overtaking started
  overtakeCooldown = 0,   -- Cooldown between overtakes (in seconds)
}

function OvertakeManager:detectCurveDirection(plan)
  -- Analyze the next few waypoints to determine curve direction
  if #plan < 3 then
    return 0  -- Not enough waypoints to detect a curve
  end
  ...
  -- Calculate the cross product to determine curve direction
  local vec1 = p2 - p1
  local vec2 = p3 - p2
  local crossProduct = vec1:cross(vec2)

  -- Return -1 for left curve, 1 for right curve, 0 for straight
  if crossProduct.z > 0 then
    return -1  -- Left curve
  elseif crossProduct.z < 0 then
    return 1   -- Right curve
  else
    return 0   -- Straight
  end
end

function OvertakeManager:canOvertake(ai, trafficTable, plan)
  -- Check if overtaking is possible based on traffic and track conditions
  if self.overtaking or os.time() - self.overtakeStartTime < self.overtakeCooldown then
    return false
  end

  -- Detect curve direction
  local curveDirection = self:detectCurveDirection(plan)

  for _, vehicle in ipairs(trafficTable) do
    local speedDifference = ai.speed - vehicle.vel:length()
    local distanceToVehicle = ai.pos:distance(vehicle.pos)

    -- Check if the vehicle ahead is slower and within a reasonable distance
    if speedDifference > 1 and distanceToVehicle < 10 then
      -- Bias overtaking side based on curve direction
      if curveDirection ~= 0 then
        self.overtakeSide = curveDirection  -- Overtake on the side of the curve
      else
        self.overtakeSide = math.random(-1, 1)  -- Randomly choose left or right for straight sections
      end
      return true
    end
  end
...
function OvertakeManager:startOvertake(ai, plan)
  self.overtaking = true
  self.overtakeStartTime = os.time()

  -- Reduce safety distance during overtakes
  M.previousSafetyDistance = M.SafetyDistance -- Saving current safetyDistance for Reset when overtake ends
  M.safetyDistance = M.safetyDistance * 0.2  -- Reduce safety distance by 80%

  -- Adjust the plan for overtaking
  for i = 1, #plan do
    local n = plan[i]
    local trackWidth = n.radiusOrig * n.chordLength * 2
    local lateralOffset = self.overtakeSide * trackWidth * 0.5  -- Move to the side
    n.lateralXnorm = clamp(n.lateralXnorm + lateralOffset, -trackWidth, trackWidth)
  end

  -- Increase aggression during overtakes
  M.previousAggression = M.aggression -- Saving current aggression for Reset when overtake ends
  M.aggression = M.aggression * 1.3  -- Increase aggression by 30%

  print("Overtaking started on side: " .. (self.overtakeSide == -1 and "left" or "right"))
end

function OvertakeManager:endOvertake(ai, plan)
  self.overtaking = false
  M.safetyDistance = M.previousSafetyDistance  -- Reset safety distance to previous value
  M.aggression = M.previousAggression -- Reset aggression to previous value

  for i = 1, #plan do
    local n = plan[i]
    n.lateralXnorm = 0  -- Reset lateral offset
  end
  print("Overtaking ended.")
end
...
local function calculateTarget(plan)
  ...
  local targetLength = baseTargetLength * speedFactor

  -- Ensure the target length is not too short or too long
  targetLength = clamp(targetLength, 4.5, 40)  -- Clamp between 4.5m and 40m

  if OvertakeManager.overtaking then
    -- Increase speed during overtaking
    ai.speed = ai.speed * 1.5  -- Increase speed by 50%
  end
  ...
  -- Apply overtaking logic if active
  if M.OvertakeManager.overtaking then
    local trackWidth = plan[targetSeg].radiusOrig * plan[targetSeg].chordLength * 2
    local lateralOffset = M.OvertakeManager.overtakeSide * trackWidth * 0.5  -- Move to the side
    targetPos:setAdd(plan[targetSeg].dirVec:cross(vec3(0, 0, 1)) * lateralOffset)
    ai.speed = ai.speed * 1.1  -- Slightly increase speed during overtake
  end
  ...
  for plID, v in pairs(mapmgr.getObjects()) do
    if plID ~= objectId and (M.mode ~= 'chase' or plID ~= player.id or chaseData.playerState == 'stopped') then
      ...
      if avoidCars == 'on' or v.targetType == 'follow' then
        v.length = obj:getObjectInitialLength(plID) -- UPDATED FUNCTION -- manually removed additional length
...
-- Purpose: Controls how closely the AI follows other vehicles or objects. Behavior: A smaller safetyDistance allows the AI to drive closer to other vehicles, which can be useful for aggressive racing or overtaking. A larger safetyDistance makes the AI maintain a larger gap, which is safer but may result in slower lap times.
...
-- Purpose: Controls how much the AI can move laterally within the track width. Behavior: A larger lateralOffsetRange allows the AI to move more freely across the track, which is useful for overtaking or avoiding obstacles. A smaller lateralOffsetRange restricts the AI to stay closer to the center of the track.
...
-- Purpose: Controls how aggressively the AI moves laterally during overtaking or cornering. Behavior: A larger lateralOffsetScale makes the AI move more aggressively to the side, which is useful for overtaking or taking optimal racing lines. A smaller lateralOffsetScale makes the AI move more conservatively, staying closer to the center of the track.
...
-- Purpose: Controls how strongly the AI prioritizes the shortest path over other factors (e.g., smoothness, safety). Behavior: A higher shortestPathBias makes the AI take the shortest possible path, which can result in sharper turns and more aggressive driving. A lower shortestPathBias allows the AI to take smoother, longer paths, which may be safer and more comfortable.
...
local function racingBehavior(route, baseRoute)
  ...
  -- Use OvertakeManager to detect and start overtakes
  if OvertakeManager:canOvertake(ai, trafficTable, route.plan) then
    OvertakeManager:startOvertake(ai, route.plan)
  end

  -- End overtake if conditions are no longer met
  if OvertakeManager.overtaking and not OvertakeManager:canOvertake(ai, trafficTable, route.plan) then
    OvertakeManager:endOvertake(ai, route.plan)
  end
end
...
local function driveUsingPath(arg)
  ...
  * examples:
  ...
  safetyDistance = arg.safetyDistance or 0.1 -- Default to 0.1 only if not provided
  lateralOffsetRange = arg.lateralOffsetRange or 0.8 -- Default to 0.8 only if not provided
  lateralOffsetScale = arg.lateralOffsetScale or 0.7 -- Default to 0.7 only if not provided
  shortestPathBias = arg.shortestPathBias or 0.8 -- Default to 0.8 only if not provided
...
--- IMPORTANT -- CUSTOM CODE BELOW
local function logDriverDataToCsv(trackName, driver, risk, vision, awareness, safetyDistance, lateralOffsetRange, lateralOffsetScale, shortestPathBias, turnForceCoef, springForceIntegratorDispLim)
  local fileName = trackName .. "_" .. driver .. ".csv"
  ...
  if not misc.csvFile then
      ...
      misc.csvFile = require('csvlib').newCSV("number", "trackName", "driver", "risk", "vision", "awareness", "safetyDistance", "lateralOffsetRange", "lateralOffsetScale", "shortestPathBias", "turnForceCoef", "springForceIntegratorDispLim")
  end

  -- Append the new data
  misc.csvFile:add(nextRowNumber, trackName, driver, risk, vision, awareness, safetyDistance, lateralOffsetRange, lateralOffsetScale, shortestPathBias, turnForceCoef, springForceIntegratorDispLim)
