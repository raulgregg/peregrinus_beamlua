...
cursor = conn.cursor()
query = """
SELECT map_name, series_name, stage_class, class_max_checkpoint
FROM map_data
WHERE series_name = 'pereRally'
"""
cursor.execute(query)
max_checkpoints_data = cursor.fetchall()
conn.close()
max_checkpoints_df = pd.DataFrame(max_checkpoints_data, columns=['map_name', 'series_name', 'stage_class', 'class_max_checkpoint'])
...
df = df.merge(max_checkpoints_df, on=['map_name', 'stage_class', 'series_name'], how='left')
...
groups = df.groupby(['driver', 'map_name', 'series_name', 'stage_class', 'run']).agg(
...
merged_df = expanded_df.merge(
    df,
    on=['driver', 'map_name', 'stage_class', 'series_name', 'run', 'checkpoint'],
    how='left'
).drop_duplicates(
