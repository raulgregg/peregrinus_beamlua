...
# Load dataset from all CSV files in the directory
...
# Manually input correct values for 'run' and 'is_training'
...
# Feature selection
features = ["driver", "map_name", "stage_class", "run", "is_training", "checkpoint", "multiplier"]
parameters = ["risk", "vision", "awareness", "safetyDistance", "lateralOffsetRange", "lateralOffsetScale", "shortestPathBias", "turnForceCoef", "springForceIntegratorDispLim"]
...
df[parameters] = df[parameters].astype(np.float32)
...
class RallyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.df.iloc[self.current_index, -len(parameters):] = self.df.iloc[self.current_index, -len(parameters):].astype(np.float32) + action.astype(np.float32)
...
for driver_col in driver_columns:
    ...
    for track_class, pred_list in predictions_by_class.items():
        ...
        map_name = next((col.replace("map_", "") for col in map_cols if meta_row[col] == 1), "UnknownMap")

        output_file_path = f"{driver_folder}/parameters_for_{map_name}_{track_class_clean}_run{adjusted_run}_{driver_clean}.csv"

        
        output_df = pd.DataFrame(pred_list)
        output_df.to_csv(output_file_path, index=False)
        print(f"Predictions saved for driver {driver_clean} and class {track_class_clean} in {output_file_path}")
