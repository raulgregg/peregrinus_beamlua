...
# Feature selection
features = ["driver_name", "map", "track_class", "car", "run", "checkpoint"]
...
df = pd.get_dummies(df, columns=["driver_name", "map", "track_class", "car"], drop_first=False)
...
class RallyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.df.iloc[self.current_index, -len(parameters):] = self.df.iloc[self.current_index, -len(parameters):].astype(np.float32) + action
