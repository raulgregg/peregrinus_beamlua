  ...
  for i = 2, #plan - 1 do
    ...
    for _, v in ipairs(trafficTable) do
      if v.vel:dot(n1.dirVec) < ai.speed * 0.6 then
        ...
        -- Reduce safety distance during overtaking
        local overtakingSafetyDistance = safetyDistance * 0.5  -- Reduce safety distance by 50%
        local side = math.random(-1, 1)
        local lateralOffset = side * (n1.radiusOrig * n1.chordLength * lateralOffsetScale)

        -- Apply lateral offset and adjust speed for overtaking
        n1.lateralXnorm = clamp(n1.lateralXnorm + lateralOffset, -n1.radiusOrig, n1.radiusOrig)
        n1.speed = ai.speed * 1.2  -- Increase speed during overtaking

        -- Ensure the AI doesn't deviate too far from the shortest path
        if shortestPathBias > 0.5 then
          n1.lateralXnorm = n1.lateralXnorm * (1 - shortestPathBias)
        end
    ...
    -- Adjust safety distance dynamically based on traffic density
    local minSqDist = math.huge
    if rnorm > 0 and rnorm < 1 and vnorm > 0 and vnorm < 1 then
      minSqDist = 0
    else
