...
local function calculateTarget(plan)
  ...
  local trafficDensity = #trafficTable
  local speedFactor = clamp(ai.speed / 30, 0.5, 2)
  local trafficFactor = 1 + (trafficDensity * 0.2)

  -- Calculate dynamic targetLength
  local targetLength = baseTargetLength * speedFactor * trafficFactor

  -- Increase target length during overtaking
  if OvertakeManager.overtaking then
    targetLength = targetLength * 1.5  -- Look further ahead during overtakes
  end

  -- Ensure the target length is not too short or too long
  targetLength = clamp(targetLength, 4.5, 40)
  ...
  if plan.planCount >= 3 then
    ...
    -- Calculate local traffic density for the current and next nodes
    local trafficRadius = 20  -- Radius to check for nearby vehicles (in meters)
    local currentTrafficDensity = getLocalTrafficDensity(plan[1].pos, trafficRadius)
    local nextTrafficDensity = getLocalTrafficDensity(plan[2].pos, trafficRadius)

    -- Adjust targetLength based on local traffic density
    local trafficFactorCurrent = 1 + (currentTrafficDensity * 0.2)
    local trafficFactorNext = 1 + (nextTrafficDensity * 0.2)
    remainingDistanceCurrentSeg = remainingDistanceCurrentSeg * trafficFactorCurrent
    weightedNextSegLength = weightedNextSegLength * trafficFactorNext
...
local function racingBehavior(route, baseRoute)
  ...
  elseif OvertakeManager.overtaking then
    OvertakeManager:endOvertake(ai, route.plan)
  end

  -- Adjust lateral movement based on overtaking state
  local plan = route.plan
  for i = 1, #plan do
    local n = plan[i]
    if not driveInLaneFlag then
      ...
      -- Add randomness for realism and overtaking
