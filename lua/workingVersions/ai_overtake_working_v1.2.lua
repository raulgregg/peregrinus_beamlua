...
local OvertakeManager = {
  ...
  overtakeCooldown = 5,   -- Cooldown between overtakes (in seconds)
}
...
function OvertakeManager:canOvertake(ai, trafficTable, plan)
  ...
  for _, vehicle in ipairs(trafficTable) do
    ...
    if speedDifference > 5 and distanceToVehicle < 20 then
...
function OvertakeManager:startOvertake(ai, plan)
  ...
  M.safetyDistance = M.safetyDistance * 0.3  -- Reduce safety distance by 70%
...
function OvertakeManager:endOvertake(ai, plan)
  ...
  M.safetyDistance = 0.5  -- Reset safety distance to default
  ...
  for plID, v in pairs(mapmgr.getObjects()) do
    if plID ~= objectId and (M.mode ~= 'chase' or plID ~= player.id or chaseData.playerState == 'stopped') then
      ...
      if avoidCars == 'on' or v.targetType == 'follow' then
        ...
        local baseSafetyDistance = 0.5  -- Default safety distance
        local speedFactor = clamp(ai.speed / 30, 0.5, 2)  -- Adjust based on speed
        local trafficDensity = #trafficTable  -- Number of nearby vehicles
        local densityFactor = 1 + (trafficDensity * 0.1)  -- Increase distance in heavy traffic
      
        -- Reduce safety distance in race mode or during overtakes
        if raceMode == 'on' or OvertakeManager.overtaking then
          baseSafetyDistance = baseSafetyDistance * 0.3  -- Reduce safety distance by 70%
        end
      
        -- Apply dynamic safety distance
        local safetyDistance = baseSafetyDistance * speedFactor * densityFactor
      
        -- Adjust vehicle positions based on dynamic safety distance
        v.posFront = dirVec * safetyDistance + posFront
        v.posRear = dirVec * (-v.length - safetyDistance) + posFront
        v.posMiddle = (v.posFront + v.posRear) * 0.5
...
local function driveUsingPath(arg)
  ...
  * examples:
  ...
  safetyDistance = arg.safetyDistance or 0.5 -- Default to 0.5 only if not provided
  lateralOffsetRange = arg.lateralOffsetRange or 0.4 -- Default to 0.4 only if not provided
  lateralOffsetScale = arg.lateralOffsetScale or 0.3 -- Default to 0.3 only if not provided
  shortestPathBias = arg.shortestPathBias or 0.7 -- Default to 0.7 only if not provided
